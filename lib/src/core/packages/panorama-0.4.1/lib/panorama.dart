...
class Panorama extends StatefulWidget {
  ...
  /// Enable hotspots' deformation on view changes.
  final bool deformHotspots;
...
class _PanoramaState extends State<Panorama>
  ...
  Vector3 positionToLatLon(double x, double y) {
    ...
    final Matrix4 m =
        scene!.camera.projectionMatrix * scene!.camera.lookAtMatrix as Matrix4;
  ...
  Vector3 positionFromLatLon(double lat, double lon) {
    ...
    final Matrix4 m = scene!.camera.projectionMatrix *
        ...
        matrixFromLatLon(lat, lon) as Matrix4;
    ...
    if (hotspots != null && scene != null) {
      for (Hotspot hotspot in hotspots) {
        ...
        final Matrix4 transform = scene!.camera.lookAtMatrix *
            matrixFromLatLon(hotspot.latitude, hotspot.longitude) as Matrix4;
        final Widget child = Positioned(
          left: pos.x - orgin.dx,
          top: pos.y - orgin.dy,
          width: hotspot.width,
          height: hotspot.height,
          child: widget.deformHotspots
              ? Transform(
                  origin: orgin,
